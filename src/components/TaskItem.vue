<template>
  <div
    class="task-item"
    :id="task.id"
    v-bind:style="{marginLeft: task.indent * 20 + 'px'}"
  ...
  >
    ...
    <div
      ...
      v-bind:style="{display:'inline-block', cursor: 'text', width:500 - task.indent * 20  + 'px', height:'1em', textDecoration: (task.checked)?'line-through':'none'}"
    >{{ task.text }}</div>
    <input
      type="text"
      ref="input"
